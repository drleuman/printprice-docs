...
The **Phase 10/35 Preflight Diagnostic Contract** is certified as **production validated**. 
...
7.  **Payload Preservation**: The canonical diagnostic payloads generated by the engine must be preserved byte-for-byte as they traverse downstream databases and BFF interfaces.
...
*   **`DEGRADED`**: The analysis finished successfully, but one or more optional extraction tools were unavailable on the host node, requiring fallback parsing.
...
seqdiag
    ...
    Service ->> BFF/App: Serves raw canonical data
    BFF/App ->> ControlPlane: Aggregates findings and governs queue locks
```
...
> [!NOTE]
> **DEVELOPMENT STAGE**: The capabilities outlined below are scheduled for future implementation and are **not** present in the active frozen production environment.
